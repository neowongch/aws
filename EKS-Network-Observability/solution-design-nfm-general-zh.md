...
EKS 生产集群的应用 Pod 与外部数据库（通过跨账号 VPC PrivateLink 端点访问）之间出现间歇性 TCP 连接延迟抖动。

当前排查手段局限于：
- 客户侧慢日志（仅应用层）
- AWS 基础设施调查（需提交工单，事后分析）

目前缺少持续的、Pod 级别的网络性能基线，无法区分 AWS 基础设施问题与应用/第三方（数据库提供商）问题。
...
```
┌─────────────────────────────────────────────────────────────────┐
│  EKS 集群 (<CLUSTER_NAME>, <REGION>)                            │
│                                                                 │
│  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐            │
│  │  应用 Pod   │  │  应用 Pod   │  │  应用 Pod   │            │
│  │  (DB 客户端)│  │  (DB 客户端)│  │  (DB 客户端)│            │
│  └──────┬──────┘  └──────┬──────┘  └──────┬──────┘            │
│         │                 │                 │                    │
│  ┌──────┴─────────────────┴─────────────────┴──────┐           │
│  │           工作节点 (<INSTANCE_TYPE>)              │           │
...
| 指标 | 说明 | 用途 |
|------|------|------|
| TCP 重传次数 (RT) | 因丢包或损坏而重新发送的数据包数 | 检测 Pod 与端点之间的丢包情况 |
...
| 无法区分 AWS 基础设施问题 vs 数据库提供商问题 | 流量指标精确显示重传发生在哪个环节 |
...
1. 在目标 EKS 集群部署 NFM
2. 建立 1 周基线：应用 Pod -> 数据库端点流量的 RT/RTO 水平
3. 设置 CloudWatch 告警：RT 峰值超过阈值（阈值待基线确定）
4. 下次发生延迟抖动时检查：
   - NHI 值（AWS 归因）
   - `bw_*_allowance_exceeded`（主机争抢证据）
   - 特定 Pod -> 端点流的 RT/RTO（定位受影响 Pod）
5. 用证据驱动解决：如带宽超限则转用专用实例；如 NHI 正常且无超限则排查数据库提供商；如有具体数据则提交 AWS 工单
